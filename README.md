...
Then follow the instructions for your platform to link rn-video into your project:
...
Run `react-native link rn-video` to link the rn-video library.
...
+  `pod 'rn-video', :path => '../node_modules/rn-video/react-native-video.podspec'`
end
```
...
```diff
  pod 'Folly', :podspec => '../node_modules/react-native/third-party-podspecs/Folly.podspec'
+  `pod 'rn-video/VideoCaching', :path => '../node_modules/rn-video/rn-video.podspec'`
end
```
...
`react-native link rn-video` doesn’t work properly with the tvOS target so we need to add the library manually.
...
If your project is using React Native < 0.60, run `react-native link rn-video` to link the rn-video library.
...
```gradle
include ':rn-video'
project(':rn-video').projectDir = new File(rootProject.projectDir, '../node_modules/rn-video/android-exoplayer')
```
...
Autolinking should automatically add rn-video to your app.
...
3. Select `node_modules\rn-video\windows\ReactNativeVideoCPP\ReactNativeVideoCPP.vcxproj`
...
import RCTVideoManager from 'rn-video/dom/RCTVideoManager'; // Add this
...
For Android MediaPlayer, you will need to build your own controls or use a package like [rn-video-controls](https://github.com/itsnubix/rn-video-controls) or [rn-video-player](https://github.com/cornedor/rn-video-player).

Note on Android ExoPlayer, native controls are available by default. If needed, you can also add your controls or use a package like [rn-video-controls].
...
At some point in the future, rn-video will include an Audio Manager for configuring how videos mix with other apps playing sounds on the device.
...
- Try the included [VideoPlayer example][2] yourself:
   ...
   ```sh
   git clone git@github.com:vobachnhac1/rn-video.git
   cd rn-video/example
   npm install
...
You will either need to install the version 27 SDK and version 27.0.3 buildtools or modify your build.gradle file to configure rn-video to use the same build settings as the rest of your app as described below.
